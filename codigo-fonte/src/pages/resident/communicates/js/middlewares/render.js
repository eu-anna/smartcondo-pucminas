...
export function renderList(limit = null) {
	...
	if (notificationsFromTheBuildingManager) {
		sentByTheBuildingResident(notificationsFromTheBuildingManager,categories,list)
	}

	//Mostra as notificações enviadas pelo morador
	if (notificationsFromLoggedUser || notificationsFromLoggedUser.length != 0) {
		sentByTheBuildingManager(notificationsFromLoggedUser,categories,myListRender);
	}
