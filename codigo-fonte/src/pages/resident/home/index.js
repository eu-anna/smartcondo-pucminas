...
window.addEventListener("DOMContentLoaded", () => {
	...
	btnNewOccurrence.addEventListener("click", () => {
		window.location.href = "../occurrence/occurrence.html";
	});
});
...
function renderModalReservation() {
	...
	const myReservation = reservation?.filter(
		(m) =>
			String(m.authorId) == String(userLogged.id) &&
			m.status !== "rejected" &&
			m.status !== "pending"
	);
	...
	myReservation.forEach((e) => {
		const card = document.createElement("div");
		const dat = new Date(e.dateInit);
		const dateInit = `${String(dat.getDate()).padStart(2, "0")}/${String(
			dat.getMonth() + 1
		).padStart(2, "0")}/${dat.getFullYear()}`;
		e.dateInit = dateInit;
